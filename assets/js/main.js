  ...
  if ($intro.length > 0) {
    ...
    images.hover(
      (event) => {
        ...
        $item.children().eq(0).addClass("image_hidden");
        $item.children().eq(1).removeClass("image_hidden");
      },
      (event) => {
        const $item = $(event.currentTarget);
        $item.children().eq(1).addClass("image_hidden");
        $item.children().eq(0).removeClass("image_hidden");
      }
    );
  }
})(jQuery);
